...
function queryData() {
	...
	$("#dataTablesDiv")
			.html(
					"<table id='table' class='common-table' width='100%' border='0' cellspacing='0'></table> ");
...
function showDetails(d) {
	var re = "<div class='container-fluid' style='margin-top:5px;'><dl class='dl-horizontal'>"
			...
			+ "<dt>请求头</dt>"
			+ "<dd>"
			+ JSON.stringify(d['request_header'])
			+ "</dd>"
			+ "<dt>请求体</dt>"
			+ "<dd>"
			+ JSON.stringify(d['request_body'])
			+ "</dd>"
			+ "<dt>返回头</dt>"
			+ "<dd>"
			+ JSON.stringify(d['response_header'])
			+ "</dd>"
			+ "<dt>请求耗时</dt>"
			+ "<dd>"
			+ d['request_time']
...
function addScan() {
	...
	var datepicker = $("#datepicker").val();
	var url = Common.Constants.DATA_INTERFACE_URL
			+ Common.Constants.P2_DATA_URL_ADDSCAN;
	var params = {
		domain : domain,
		time : datepicker
	};
