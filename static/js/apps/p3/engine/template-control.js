...
function formatOp(data, type, full) {
    ...
    var re = '<a class="delete"><i class="fa fa-trash-o" style=\"color:#18A689\"></i>删除</a>&nbsp;' + '<a class="installModule"><i class="fa fa-wrench" style="color:#18A689"></i>配置扫描引擎</a>';
    return re;
};
...
function templateControlDelete() {
    ...
    $(".delete").click(function() {
        ...
        $('button#submit').click(function(event) {
            event.stopPropagation();
            var url = Common.Constants.DATA_INTERFACE_URL + Common.Constants.TEMPLATE_DELETE;
            var params = [{
                "name": "template_id",
                "value": template_id
            }];
            loadDataByAjax(url, params, delteSuccess);
        });
    });
}

function delteSuccess(data, textStatus) {
...
function templateControlPlus() {
    ...
    $('button#moduleAdd').click(function(event) {
        ...
        var url = Common.Constants.DATA_INTERFACE_URL + Common.Constants.TEMPLATE_Add;
        var params = [{
            "name": "template_name",
            "value": template_name
        }];
...
function engineModuleLoad(data) {
...
function templateControlConfiguration() {
    ...
    $('.installModule').click(function() {
        ...
        var params = [];
        if (flag) {
            loadDataByAjax(url, params, engineModuleLoad);
        }
    ...
    $('#moduleInstall').click(function() {
        ...
        var url = Common.Constants.DATA_INTERFACE_URL + Common.Constants.TEMPLATE_CONFIGTEMPLATE;
